...
library(rgdal)
library(raster)
setwd("Y:/Matt Yap Work folder/Aggregation simulations/Cholera maps")

# Rates.Raster <- raster ("http://www.iddynamics.jhsph.edu/sites/default/files/content/page/rate.tif")
Cases.Raster <- raster("http://www.iddynamics.jhsph.edu/sites/default/files/content/page/case.tif")

# africa_shp <- readOGR(dsn="africa.shp", layer="africa")
# library(maptools)
# africa_country <- unionSpatialPolygons(
	# africa_shp,
	# IDs = africa_shp$NAME_0)

## Now we have the data, we need to simulate distributions of the cases at cells smaller than this.
popdata <- raster("Y:/Matt Yap Work folder/Aggregation simulations/Cholera maps/Africa_1km_Population/AFR_PPP_2015_adj_v2.tif")
(pop_1km <- cellStats(popdata, 'sum'))/1000000
## 1.186 b in 2015
# crs(Rates.Raster) <- crs(popdata)	## This assumes that the project of the Case raster is the same as the worldpop raster
crs(Cases.Raster) <- crs(popdata)
## = 9.906 cases per 100,000 (appears that each case is replicated through each 1km grid cell)
class(Rates.resampled); class(popdata)

### Now I have the data, I will do the rest of the analysis on a small sample.
sample.extent <- c(xmin=10, xmax=11.5, ymin=5, ymax=6.5)

## checking characteristics
extent(popdata); extent(Rates.Raster)
crs(popdata); crs(Rates.Raster)


## If they are the same, I can then create my samples
sample.pop <- crop(popdata, sample.extent)		# 1km
# sample.rates <- crop(Rates.Raster, sample.extent)	#20km
sample.cases <- crop(Cases.Raster, sample.extent)
par(mfrow=c(2,2))
plot(log10(popdata)) + plot(extent(sample.extent), add=TRUE)	+ title("Original pop (1km)")
# plot(log10(Rates.Raster)) + plot(extent(sample.extent), add=TRUE) + title("Original rates (20km)")
plot(log10(Cases.Raster)) + plot(extent(sample.extent), add=TRUE) + title("Original cases (20km)")
plot(log10(sample.pop))
# plot(log10(sample.rates))
plot(log10(sample.cases))
...
MasterDF$pop <- as.vector(sample.pop)
MasterDF$pop[is.na(MasterDF$pop)] <- 0
table(complete.cases(MasterDF$pop))
...
library(doParallel)
registerDoParallel(clus <- makeCluster(min(50,num_zonations)))
clusterExport(clus,c("seeds","adj.tmp","MasterDF2","clusterList","clusterPop","clusterNo","cellNo", "targetPop","size"))

test <- foreach(curr_seed = 1:num_zonations) %dopar% {
	...
	repeat {
		...
		# if ((clusterNo %% 100)==0){
			# print(paste0("Big grids:",size,"x",size,".Percent of cells completed: ",round(length(unlist(clusterList))/(nsupergrid.r * nsupergrid.c * ntinygrid.r * ntinygrid.c)*100),"%")); flush.console()
		# }
	}
	...
	return(list(
		"MasterDF2" = MasterDF2,
		"cluster.df2" = cluster.df2))
	
}		#### End parallel here		<<<---------	<<<---------
### parallel loop started at 11:32am for 100 zonations
